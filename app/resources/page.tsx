            ...
            {section.type === 'external_links' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {section.content.map((category: any) => (
                  <div key={category.id} className={`bg-white/40 md:${getColorClasses(category.color)} rounded-lg p-4 md:p-6 border ${category.color === 'blue' ? 'border-blue-200' : 'border-green-200'}`}>
                    ...
                    <div className="space-y-2 md:space-y-3">
                      {category.links.map((link: any, index: number) => (
                        <a 
                          ...
                          className="block bg-white/70 md:bg-white/50 rounded p-3 hover:bg-white/80 md:hover:bg-white/70 transition-colors border border-amber-200/50 md:border-none"
                        >
                          <h5 className="font-semibold text-amber-700 text-sm md:text-base">{link.title}</h5>
                          <p className="text-xs text-amber-600 leading-relaxed">{link.description}</p>
                          <span className="text-blue-600 text-xs">→ {link.display_url}</span>
                        </a>
                      ))}
                    </div>
                  </div>
            ...
            {section.type === 'museums_sites' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                {section.content.map((site: any) => (
                  <div key={site.id} className={`bg-white/40 md:${getColorClasses(site.color)} rounded-lg p-4 md:p-6 border ${site.color === 'orange' ? 'border-orange-200' : site.color === 'teal' ? 'border-teal-200' : 'border-red-200'}`}>
                    <h4 className="font-bold text-amber-800 mb-3 text-center text-base md:text-lg flex items-center justify-center">
                      {getCategoryIconComponent(site.id)}
                      {site.title}
                    </h4>
                    <div className="space-y-2">
                      {site.features.map((feature: any, index: number) => (
                        <div key={index} className="bg-white/70 md:bg-white/50 rounded p-3 border border-amber-200/50 md:border-none">
                          <h5 className="font-semibold text-amber-700 text-sm">{feature.title}</h5>
                          <p className="text-xs leading-relaxed">{feature.description}</p>
                          {feature.button && (
                            <button className={`${getButtonColorClass(feature.button_color)} text-white px-2 py-1 rounded text-xs mt-1 transition-colors`}>
                              {feature.description}
                            </button>
                          )}
                        </div>
